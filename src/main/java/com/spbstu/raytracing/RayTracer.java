...
public class RayTracer {
    ...
    public Color getDiffuse(final IntersectionInfo intersectionInfo, final LightSource lightSource, final double intensityFactor) {
        ...
        try {
            ...
            return intersectionInfo.getColor(Color.BLACK, new Color((int) (color.getRed() * c), (int) (color.getGreen() * c), (int) (color.getBlue() * c)), Color.BLACK);
    ...
    public Color getSpecular(final IntersectionInfo intersectionInfo, final Ray ray, LightSource lightSource, final double intensityFactor) {
        ...
        switch (rayTracerInfo.lightningStyle) {
            ...
            case PHONG_BLINN:
                ...
                if (Math.abs(intersectionInfo.getPoint().getX()) == 0 && intersectionInfo.getPoint().getZ() == -0.5) {
        ...
        Color color = lightSource.getColor();
        return intersectionInfo.getColor(Color.BLACK, Color.BLACK, new Color((int) (color.getRed() * c), (int) (color.getGreen() * c), (int) (color.getBlue() * c)));
    }
    ...
    public Color getColor(final Ray ray, final double refractionIndex, final double intensity, final int deepLevel) {
        ...
        if (intersectionInfo != null) {
            Color ambient = intersectionInfo.getColor(new Color((int) (intensity * rayTracerInfo.ambient.getRed()), (int) (intensity * rayTracerInfo.ambient.getGreen()), (int) (intensity * rayTracerInfo.ambient.getBlue())), Color.BLACK, Color.BLACK);
            ...
            resultColor = add(ambient, diffuse, specular);
    ...
    boolean isInShadow(final IntersectionInfo intersectionInfo, final LightSource lightSource) {
        ...
        Point intersectionPoint = intersectionInfo.getPoint();
        Ray toLightSourceRay = new Ray(Point.translate(intersectionPoint, Vector.onNumber(intersectionInfo.getNormal(), 1e-5)), Vector.onNumber(lightSource.getOnPointDirection(intersectionPoint), -1).getNormalized());
    ...
    public void apply() throws InterruptedException {
        ...
        int threadsNumber = 1;//Runtime.getRuntime().availableProcessors();
